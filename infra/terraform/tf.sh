...
# Pourquoi : sous Windows, un antivirus qui intercepte le TLS (Avast) casse la communication
# chiffrée entre Terraform et ses providers ; dans un conteneur, ce trafic reste interne.
# Le CA de l'antivirus est ajouté au bundle du conteneur pour les appels sortants
# (registry, AWS, checkip). La racine du dépôt est montée pour que Terraform lise contexte/fraudTest.csv.
#
...
ARGS=(--rm -v "$ROOT_W:/repo" -w /repo/infra/terraform -v "$AWS_DIR:/root/.aws:ro" -e AWS_PROFILE="${AWS_PROFILE:-default}")
[[ -t 0 ]] && ARGS+=(-it)
